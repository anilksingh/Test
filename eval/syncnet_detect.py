...
class SyncNetDetector:
    ...
    def detect_face(self, frames_dir, facedet_scale=0.25):
        ...
        for fidx, fname in enumerate(flist):
            ...
            if bboxes:  # If faces are detected, return the detections immediately
                print(f"Face detected in frame {fidx}")
                return [{"frame": fidx, "bbox": (bbox[:-1]).tolist(), "conf": bbox[-1]} for bbox in bboxes]

            print(f"No face detected in frame {fidx}")

        return []  # Return an empty list if no face is detected in any frame
