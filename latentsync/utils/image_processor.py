...
class ImageProcessor:
    ...
    def affine_transform(self, image: torch.Tensor, allow_multi_faces: bool = True) -> np.ndarray:
        ...
        if self.fa is None:
            ...
            if not allow_multi_faces and len(detected_faces) > 1:
                return None, None, None  # Return None if multiple faces are detected and not allowed
    ...
    def prepare_masks_and_masked_images(self, images: Union[torch.Tensor, np.ndarray], affine_transform=False):
        # Ensure that images are converted to a tensor if they are not already
        if isinstance(images, list):
            images = np.array(images)  # Convert list to numpy array

        if isinstance(images, np.ndarray):
            images = torch.from_numpy(images)  # Convert numpy array to tensor
